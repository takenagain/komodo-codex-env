...
main() {
    ...
    if docker exec -u testuser "$CONTAINER_ID" bash -c "source ~/.bashrc && command -v fvm"; then
        log_success "FVM command is available"
    else
        log_error "FVM command not found"
        log_info "Checking PATH and installed packages..."
        docker exec -u testuser "$CONTAINER_ID" bash -c "echo 'PATH: $PATH'; ls -la ~/.pub-cache/bin/ 2>/dev/null || echo 'No .pub-cache/bin directory'"
        exit 1
    fi
    
    # Verify Flutter installation via FVM
    log_info "Verifying Flutter installation via FVM..."
    if docker exec -u testuser "$CONTAINER_ID" bash -c "source ~/.bashrc && fvm flutter --version"; then
        log_success "Flutter is available via FVM"
        
        # Check the specific version
        log_info "Checking Flutter version..."
        FLUTTER_VERSION_OUTPUT=$(docker exec -u testuser "$CONTAINER_ID" bash -c "source ~/.bashrc && fvm flutter --version 2>&1" | head -1)
        ...
        docker exec -u testuser "$CONTAINER_ID" bash -c "source ~/.bashrc && fvm list || echo 'fvm list failed'"
