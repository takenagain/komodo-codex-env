...
class DocumentationManager:
    ...
    async def fetch_all_documentation(self) -> Dict[str, str]:
        ...
        for i, result in enumerate(results):
            task_name = list(self.doc_sources.keys())[i]
    ...
    def check_for_script_updates(self) -> Optional[str]:
        ...
        try:
            response = requests.get(self.config.script_gist_url, timeout=10)
            response.raise_for_status()
    ...
    def download_script_update(self, target_path: Path) -> bool:
        ...
        try:
            response = requests.get(self.config.script_gist_url, timeout=30)
            response.raise_for_status()
