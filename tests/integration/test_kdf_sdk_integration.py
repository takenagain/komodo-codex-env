...
This test ensures the melos CLI is installed when running the setup with
``--install-type KDF-SDK`` inside a Docker container.
"""
...
class KdfSdkIntegrationTest(unittest.TestCase):
    """Test KDF-SDK setup and melos installation inside Docker."""
    ...
    def setUp(self):
        ...
        # Create testuser with sudo privileges
        self._run_command(
            [
                "docker",
                "exec",
    ...
    def tearDown(self):
        if hasattr(self, "container_id"):
            logger.info("Cleaning up container: %s", self.container_id[:12])
            subprocess.run(
    ...
    def test_kdf_sdk_installs_melos(self):
        """Install KDF-SDK type and verify melos is available."""
        ...
        try:
            ...
            # Step 3: verify melos
            logger.info("Step 3: Checking melos installation")
            result = self._run_in_container("melos --version")
            self.assertEqual(result.returncode, 0, "melos not installed")
            logger.info("✓ Melos is available")
        except Exception as e:  # pragma: no cover - integration safeguard
